...
class PhysicalValidator:
    ...
    def apply_illumination(self, img, uid, steps):
        """Simulates extreme low-light/over-exposure via Gamma Correction."""
        for alpha in steps:
    ...
    def apply_resolution(self, img, uid, steps):
        """Simulates extreme pixelation (DPI loss)."""
        h, w = img.shape[:2]
        for scale in steps:
    ...
    def apply_motion_blur(self, img, uid, steps):
        """Simulates extreme motion blur (Conveyor/Vibration)."""
        for size in steps:
    ...
    def apply_defocus(self, img, uid, steps):
        """Simulates deep optical defocus."""
        for sigma in steps:
    ...
    def apply_sensor_noise(self, img, uid, steps):
        """Simulates extreme sensor thermal noise (ISO)."""
